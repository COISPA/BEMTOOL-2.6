# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtReport/updateBiologicalfromReport.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: updateBiologicalfromReportpopuls, IsY, source_obj, yy, mm, mort_const, year_interact) {')
...
fR <- convert_numeric0_to_na(as.numeric(as.character((source_obj$ catches_nb  [1:num_classes, yy])))) 
...
numbers_temp <-  data.frame(matrix(0, ncol=(num_classes+first_age), nrow=1))
colnames(numbers_temp) <- c(paste("age", c(0:(num_classes+first_age-1)), sep=""))
fR <- convert_numeric0_to_na(as.numeric(as.character((source_obj$ landings_nb  [1:num_classes, yy])))) 
numbers_temp[1,which(colnames(numbers_temp) == paste("age",first_age, sep="")):(num_classes+first_age)] <- fR  
...
numbers_temp <-  data.frame(matrix(0, ncol=(num_classes+first_age), nrow=1))
colnames(numbers_temp) <- c(paste("age", c(0:(num_classes+first_age-1)), sep=""))
fR <- convert_numeric0_to_na(as.numeric(as.character((source_obj$ discards_nb  [1:num_classes, yy])))) 
numbers_temp[1,which(colnames(numbers_temp) == paste("age",first_age, sep="")):(num_classes+first_age)] <- fR  
...
min_ <- convert_numeric0_to_na(as.numeric(as.character(source_obj$ age_rangeF[1,1] )))
max_ <- convert_numeric0_to_na(as.numeric(as.character(source_obj$ age_rangeF[1,2] )))
...
#fR <- convert_numeric0_to_na(as.numeric(as.character((source_obj$ critical_length  [1, 3]))))
...
for (month in 1:length(MONTHS) ) { weights_temp_ex[which(rownames(weights_temp_ex) == paste("age",first_age, sep="")):(num_classes+first_age),month] <- convert_numeric0_to_na(as.numeric(as.character(fR[,yy]))) }
...
tot_numb_catch <- convert_numeric0_to_na(as.numeric(as.character((source_obj$ catches_nb  [,yy]))))                                                          
