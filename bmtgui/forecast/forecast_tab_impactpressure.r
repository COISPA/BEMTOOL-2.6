# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/bmtgui/forecast/forecast_tab_impactpressure.r'), '\n')
...
img_yield_path_fore <-   paste(mat_cfg_general[1,3],  "/", name_this_scenario  ,"Diagnosis/Biological Pressure Impact/", mat_cfg_general[1,2], " - Catch by stock ALL ", name_this_scenario,".jpg", sep="") 
img_yield_pb_fore <- try(gdkPixbufNewFromFileTrim(gsub(" +\\.", ".",img_yield_path_fore)), silent = TRUE)
...
try(
  ...
  if (!inherits(img_yield_pb_fore,'try-error')) {
    

    img_yield_fore <- gtkImageNewFromPixbuf(img_yield_pb_fore)
