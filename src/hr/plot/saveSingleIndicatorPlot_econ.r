# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/hr/plot/saveSingleIndicatorPlot_econ.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: saveSingleIndicatorPlot_econall_the_years, path_eco, path_eco_quantiles) {')
...
ECOIndicators_quantiles <- suppressWarnings(try(data.frame(read.csv(path_eco_quantiles, sep=";",header=T), stringsAsFactors=F),silent = T))
...
y_lab <- indicator_name
    ...
    y_to_plot <- convert_numeric0_to_na(as.numeric(as.character(one_indicator_fl$Value)))
#        to_be_plotted <- which(!is.na(y_to_plot))
            y_to_plot[which(is.na(y_to_plot) | y_to_plot==-Inf | y_to_plot==Inf)]  <- 0
...
if (phase == "FORECAST" & exists("one_indicator_fl_inf") ) {
     y_to_plot_inf <- convert_numeric0_to_na(as.numeric(as.character(one_indicator_fl_inf$Value)))
          y_to_plot_sup <- convert_numeric0_to_na(as.numeric(as.character(one_indicator_fl_sup$Value)))
          y_to_plot_inf_025 <- convert_numeric0_to_na(as.numeric(as.character(one_indicator_fl_inf_025$Value)))
          y_to_plot_sup_075 <- convert_numeric0_to_na(as.numeric(as.character(one_indicator_fl_sup_075$Value)))
...
# if (printOK) cat(paste(" [", indicators_to_plot_names[ind], "] ", sep="") )                                  
#}
