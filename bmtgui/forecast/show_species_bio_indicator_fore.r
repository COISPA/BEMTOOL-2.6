# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/bmtgui/forecast/show_species_bio_indicator_fore.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: show_species_bio_indicator_forew) {')
...
img_bio_indicator_pb_fore <<-  try(gdkPixbufNewFromFileTrim(bio_indicator_path_fore), silent = TRUE)

try(

  if (!inherits(img_bio_indicator_pb_fore,'try-error')) {
    
    img_bio_indicator_pb_fore_resized <- gdkPixbufScaleSimple(img_bio_indicator_pb_fore, 300, 300, "bilinear")
    img_bio_indicator_fore <- gtkImageNewFromPixbuf(img_bio_indicator_pb_fore_resized)
