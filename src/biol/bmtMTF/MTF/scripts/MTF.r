# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtMTF/MTF/scripts/MTF.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: MTFcod,avg_y,year_start,year_end,SRR,model,F01,Fsqsetting,ref_year)  {')
...
geomean <- prod(rec(cod)[,ac(min(year_end,year_end-avg_y+1):year_end)]) ^ (1/avg_y)
hke.ltf.niters <- fwd(hke.ltf.niters, control=LTFFs, sr= list(model = "geomean", params = FLPar(geomean)),
...
#------------------------------
Fbar <- iterators:: iter(constantF01@.Data,1:length(proj.years))
Fbar=as.vector(Fbar[["state"]][["obj"]])#!!
...
geomean <- prod(rec(cod)[,ac(min(year_end-avg_y+1,year_end):year_end)]) ^ (1/avg_y)
