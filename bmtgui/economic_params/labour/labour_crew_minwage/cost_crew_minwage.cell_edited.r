# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/bmtgui/economic_params/labour/labour_crew_minwage/cost_crew_minwage.cell_edited.r'), '\n')
...
bmt_cost_crew_minwage.cell_edited <- function(cell, path.string, new.text, data) {
 ...
 bmt_cost_crew_minwage.model <- data 
  ...
  bmt_fleet.cost_crew_minwage[r,] <<- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(bmt_cost_crew_minwage_list[[r]]))))
  }
