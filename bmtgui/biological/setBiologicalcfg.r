# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/bmtgui/biological/setBiologicalcfg.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: setBiological_species_settingsw) {')
...
if (!is.na(as.numeric(as.character(gtkEntryGetText(entry_biosettings_avg_years)) ) ) )  {
           ...
           end_ <<- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(mat_cfg_general[mat_cfg_general[,1] == "casestudy.endsimulation",2])))) 
          start_ <<- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(mat_cfg_general[mat_cfg_general[,1] == "casestudy.startsimulation",2])))) 
...
if (!is.null(selected_ass)) {
   ...
   if (selected_ass == "XSA")  {
        ...
        mat_temp <<- mat_cfg_XSA_list[[which(BMT_SPECIES == selected)]] 
          numb_ages_xsa <- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(gtkEntryGetText(entry_assessment_numberAgeClasses_XSA))))) 
              ...
              if (!is.na(numb_ages_xsa) ) {
               if ( numb_ages_xsa > max(toadd_male_lifespan, toadd_female_lifespan)) {
                    ...
                    mat_temp[2,6] <- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(gtkEntryGetText(entry_assessment_numberAgeClasses_XSA))))) 
                    all_OK_values <- c(all_OK_values, T)
                    }
                  } else {
                     all_OK_values <- c(all_OK_values, F)
      ...
      # checks in VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT VIT             
        ...
        mat_temp <-  mat_cfg_VIT_list[[which(BMT_SPECIES == selected)]]
          ...
          min_ <- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(gtkEntryGetText(entry_assessment_minAge_VIT))))) 
           max_ <- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(gtkEntryGetText(entry_assessment_maxAge_VIT))))) 
        ...
        if (!is.na(min_ ) ) { 
          ...
          if (gtkToggleButtonGetActive(bmt_chk_assessment_discard_VIT)) {
           ...
           mat_temp <- mat_cfg_SURBA_list[[which(BMT_SPECIES == selected)]] 
            ...
            numb_ages_surba <- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(gtkEntryGetText(entry_assessment_numberAgeClasses_SURBA))))) 
         ...
         mat_temp <- mat_cfg_externalReport_list[[which(BMT_SPECIES == selected)]] 
            ...
            numb_ages_report <- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(gtkEntryGetText(entry_assessment_numberAgeClasses_Report))))) 
...
# if (printOK)   print('Calling function: assign_bio_externalRPspathw) {')
...
# if (printOK)   print('Calling function: assign_bio_Reportresults_pathw) {')
...
# if (printOK)   print('Calling function: assign_bio_SURBAresults_pathw) {')
...
# if (printOK)   print('Calling function: assign_bio_XSAresults_pathw) {')
...
assign_bio_XSAresults_path <- function(w) {
      path <- choose.files(default = "", caption = "")
      if ( !is.na(path) )  {
# if (printOK)   print('Calling function: assign_bio_assess_Catchfleet_pathw) {')
...
assign_bio_assess_Catchfleet_path <- function(w) {
      path <- choose.files(default = "", caption = "")
      if ( !is.na(path) )  {
# if (printOK)   print('Calling function: assign_bio_assess_Ffleet_pathw) {')
...
# if (printOK)   print('Calling function: assign_bio_assess_RPs_pathw) {')
      path <- choose.files(default = "", caption = "")
      if ( !is.na(path) )  {
...
# if (printOK)   print('Calling function: assign_bio_assess_VIT_pathw) {')
      path <- choose.files(default = "", caption = "")
      if ( !is.na(path) )  {
