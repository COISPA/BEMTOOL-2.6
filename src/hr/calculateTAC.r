# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/hr/calculateTAC.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: calculateTAC) {')
                                                                   
option_TAC <<- as.numeric( as.character(cfg[rownames(cfg) == "casestudy.HR6",1] ) )
species_TAC <<-  as.numeric( substring(as.character(cfg[rownames(cfg) == "casestudy.HR6",2] ), 1, nchar(as.character(cfg[rownames(cfg) == "casestudy.HR6",2] )) ) )
...
costant <- min(0.5, 1-M) 
F_MSY <- convert_numeric0_to_na(as.numeric(as.character(reduction_by_stock_tbl$Reference_point[species_TAC])))
B_MSY <- convert_numeric0_to_na(as.numeric(as.character(reduction_by_stock_tbl$SSBref[species_TAC])))
...
B_MSY <- convert_numeric0_to_na(as.numeric(as.character(reduction_by_stock_tbl$SSBref[species_TAC])))
B <- convert_numeric0_to_na(as.numeric(as.character(reduction_by_stock_tbl$SSBcurrent[species_TAC])))
