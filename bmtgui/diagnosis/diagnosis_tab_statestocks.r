# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/bmtgui/diagnosis/diagnosis_tab_statestocks.r'), '\n')
...
img_KobePlot_pb <<- try(gdkPixbufNewFromFileTrim(img_kobe_path), silent = TRUE)
...
try(
if (!is.null(img_KobePlot_pb)) {
kb_sw$destroy()
kb_sw <<- gtkScrolledWindowNew(NULL, NULL)
kb_sw$setShadowType("etched-in")
kb_sw$setPolicy("automatic", "automatic")
kb_sw$SetUsize(350, 350)    
img_KobePlot <<- gtkImageNewFromPixbuf(img_KobePlot_pb)
...
bi_sw <<- try(gtkScrolledWindowNew(NULL, NULL),silent = T)
...
stock_reduction.sw <<-try( gtkScrolledWindowNew(NULL, NULL),silent = T)
