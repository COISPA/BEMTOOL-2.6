# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/econ/core/BMTFlbeh.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: BMTFlbehoption, Flyear, fdmat, eimat, admat, tpmat, year_to_change, simperiod, n_fleet) {')
...
#  Fleet dynamics
  if (option[6] == 1) { #        Fishrent    (unique option)
    ...
    bmtfleetdyn_share_profit_COL3 <- convert_numeric0_to_na(as.numeric(as.character(fdmat[3,i])))
                                                               # bmtfleetdyn_prev_total_revenues = as.numeric(as.character( Fleetyear[[58]]@fleetsegments[[1]]@total.revenues.CI.perc[3]  ))  
   # bmtfleetdyn_prev_total_revenues  <- convert_numeric0_to_na(as.numeric(as.character(Flyear[[year_to_change-1]]@fleetsegments[[i]]@total.revenues )))
    bmtfleetdyn_prev_total_revenues <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[year_to_change-1]]@fleetsegments[[i]]@total.revenues.CI.perc[3]  )))     
                                                               #  bmtfleetdyn_prev_vessels = as.numeric(as.character( Fleetyear[[58]]@fleetsegments[[1]]@VESSELS.annual ))
    bmtfleetdyn_prev_vessels <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[year_to_change-1]]@fleetsegments[[i]]@VESSELS.annual )))
                                                                # bmtfleetdyn_prev_BER = as.numeric(as.character( Fleetyear[[58]]@fleetsegments[[1]]@EC.BER.CI.perc[3]  ))
   # bmtfleetdyn_prev_BER <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[year_to_change-1]]@fleetsegments[[i]]@EC.BER )))        
    bmtfleetdyn_prev_BER <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[year_to_change-1]]@fleetsegments[[i]]@EC.BER.CI.perc[3]  )))       
                                                             #    bmtfleetdyn_curr_invest_number = as.numeric(as.character( Fleetyear[[59]]@fleetsegments[[1]]@investment$number ))
   # bmtfleetdyn_curr_invest_number <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[year_to_change]]@fleetsegments[[i]]@investment$number )))
    bmtfleetdyn_curr_invest_number <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[year_to_change]]@fleetsegments[[i]]@investment$number )))
...
#  Activity dynaimcs
  if (option[7] == 1) { # Fishrent   
    ...
    bmtfleetact_profitability_COL3 <- convert_numeric0_to_na(as.numeric(as.character(admat[3,i])))
                                                                            
#   bmtfleetact_prev_total_revenues <- convert_numeric0_to_na(as.numeric(as.character( Fleetyear[[58]]@fleetsegments[[1]]@total.revenues.CI.perc[3]  )))
#    bmtfleetact_prev_BER <- convert_numeric0_to_na(as.numeric(as.character( Fleetyear[[58]]@fleetsegments[[1]]@EC.BER.CI.perc[3]  )))        
#    bmtfleetact_prev_average_days <- convert_numeric0_to_na(as.numeric(as.character( Fleetyear[[58]]@fleetsegments[[1]]@DAYS.average.annual )))
#    bmtfleetact_prev_MAX_average_days <- convert_numeric0_to_na(as.numeric(as.character( Fleetyear[[58]]@fleetsegments[[1]]@MAXDAYS.average.annual  )))                                                                    
                                                                            
    bmtfleetact_prev_total_revenues <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[year_to_change-1]]@fleetsegments[[i]]@total.revenues.CI.perc[3]  )))
    bmtfleetact_prev_BER <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[year_to_change-1]]@fleetsegments[[i]]@EC.BER.CI.perc[3]  )))        
    bmtfleetact_prev_average_days <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[year_to_change-1]]@fleetsegments[[i]]@DAYS.average.annual )))
    bmtfleetact_prev_MAX_average_days <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[year_to_change-1]]@fleetsegments[[i]]@MAXDAYS.average.annual  )))
...
#  Technological progress
  if (option[8] == 1) { # Mefisto
    for (i in to_be_updated) {
     ...
     bmttechprogress_initial <- convert_numeric0_to_na(as.numeric(as.character(tpmat[1,i])))
      bmttechprogress_fraction_time <- convert_numeric0_to_na(as.numeric(as.character(tpmat[2,i])))
      bmttechprogress_invested_capital <- convert_numeric0_to_na(as.numeric(as.character(tpmat[3,i])))
      bmttechprogress_prev_capital <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[year_to_change-1]]@fleetsegments[[i]]@capital.value )))
      bmttechprogress_firstyear_capital <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[1]]@fleetsegments[[i]]@capital.value )))
    ...
    for (i in to_be_updated) {
      ...
      bmttechprogress_firstyear <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[1]]@fleetsegments[[i]]@technology )))
