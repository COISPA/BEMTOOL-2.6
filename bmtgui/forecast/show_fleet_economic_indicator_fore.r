# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/bmtgui/forecast/show_fleet_economic_indicator_fore.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: show_fleet_economic_indicator_forew) {')
...
img_RL_pb_fore <-  try(gdkPixbufNewFromFileTrim(img_RL_path_fore), silent = TRUE)

try(

  if (!inherits(img_RL_pb_fore,'try-error')) {
    
    img_RL_pb_fore_resized <- gdkPixbufScaleSimple(img_RL_pb_fore, 300, 300, "bilinear")
    img_RL_fore <- gtkImageNewFromPixbuf(img_RL_pb_fore_resized)
...
img_PS_pb_fore <- try(gdkPixbufNewFromFileTrim(img_PS_path_fore), silent = TRUE)

try(

  if (!inherits(img_PS_pb_fore,'try-error')) {
    
    img_PS_pb_fore_resized <- gdkPixbufScaleSimple(img_PS_pb_fore, 300, 300, "bilinear")
    img_PS_fore <- gtkImageNewFromPixbuf(img_PS_pb_fore_resized)
...
img_SE_pb_fore <- try(gdkPixbufNewFromFileTrim(img_SE_path_fore), silent = TRUE)

try(

  if (!inherits(img_SE_pb_fore,'try-error')) {
    
    img_SE_pb_fore_resized <- gdkPixbufScaleSimple(img_SE_pb_fore, 300, 300, "bilinear")
    img_SE_fore <- gtkImageNewFromPixbuf(img_SE_pb_fore_resized)
...
img_RB_pb_fore <-  try(gdkPixbufNewFromFileTrim(img_RB_path_fore), silent = TRUE)

try(
  
  if (!inherits(img_RB_pb_fore,'try-error')) {
    
    img_RB_pb_fore_resized <- gdkPixbufScaleSimple(img_RB_pb_fore, 300, 300, "bilinear")
    img_RB_fore <- gtkImageNewFromPixbuf(img_RB_pb_fore_resized)
