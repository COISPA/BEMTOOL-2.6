# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/econ/setEffortVarsForecast.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: setEffortVarsForecastFyear) {')
   ...
   print("Then, DAYS are equal to zero once the TAC has been reached!", quote=F)
    ...
    for (n_int in 1:n_fleet) {     
      for (yy_f in 1:foreperiod) {
        yy <- simperiod+yy_f
        Fyear[[yy]]@fleetsegments[[n_int]]@VESSELS[1,] <- convert_numeric0_to_na(as.numeric(as.character(Fyear[[simperiod]]@fleetsegments[[n_int]]@VESSELS[1,])))
        Fyear[[yy]]@fleetsegments[[n_int]]@GT.average[1,] <- convert_numeric0_to_na(as.numeric(as.character(Fyear[[simperiod]]@fleetsegments[[n_int]]@GT.average[1,])))
        Fyear[[yy]]@fleetsegments[[n_int]]@DAYS.average[1,] <- convert_numeric0_to_na(as.numeric(as.character(Fyear[[simperiod]]@fleetsegments[[n_int]]@DAYS.average[1,]))) 
        Fyear[[yy]]@fleetsegments[[n_int]]@KW.average[1,] <- convert_numeric0_to_na(as.numeric(as.character(Fyear[[simperiod]]@fleetsegments[[n_int]]@KW.average[1,]))) 
      }
    }
  ...
  if (MEY_FLEETSEGMENT == "ALL") { 
    ...
    for (n_int in 1:n_fleet) {     
      for (yy_f in 1:foreperiod) {
        ...
        Fyear[[yy]]@fleetsegments[[n_int]]@VESSELS[1,] <- convert_numeric0_to_na(as.numeric(as.character(Fyear[[simperiod]]@fleetsegments[[n_int]]@VESSELS[1,]))) * levs[MEY_LEVEL]
        } else {
        Fyear[[yy]]@fleetsegments[[n_int]]@VESSELS[1,] <- convert_numeric0_to_na(as.numeric(as.character(Fyear[[simperiod]]@fleetsegments[[n_int]]@VESSELS[1,])))
        }
        
        if (MEY_EFFORT_VAR == "DAYS") { 
        Fyear[[yy]]@fleetsegments[[n_int]]@DAYS.average[1,] <- convert_numeric0_to_na(as.numeric(as.character(Fyear[[simperiod]]@fleetsegments[[n_int]]@DAYS.average[1,]))) * levs[MEY_LEVEL]
        } else {
        Fyear[[yy]]@fleetsegments[[n_int]]@DAYS.average[1,] <- convert_numeric0_to_na(as.numeric(as.character(Fyear[[simperiod]]@fleetsegments[[n_int]]@DAYS.average[1,]))) 
        }
