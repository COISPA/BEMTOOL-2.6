# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/econ/runEcon.fore.r'), '\n')
...
# estimate of capital value and employment as well
    ...
    for (i in 1:n_fleet) {  
#      Fyear[[year_to_be_updated]]@fleetsegments[[i]]@capital.value <- convert_numeric0_to_na(as.numeric(as.character(Fyear[[year_to_be_updated-1]]@fleetsegments[[i]]@capital.value)))-as.numeric(as.character(Fyear[[year_to_be_updated-1]]@fleetsegments[[i]]@capital.cost[2]))+as.numeric(as.character(Fyear[[year_to_be_updated-1]]@fleetsegments[[i]]@investment[2]))
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
Fyear[[year_to_be_updated]]@fleetsegments[[i]]@employment <- convert_numeric0_to_na(as.numeric(as.character(eimat[7,i]))) * as.numeric(as.character(Fyear[[year_to_be_updated]]@fleetsegments[[i]]@VESSELS.annual))
# -----------------------------------------------------------------------------      
Fyear[[year_to_be_updated]]@fleetsegments[[i]]@MAXDAYS.average.annual <- convert_numeric0_to_na(as.numeric(as.character(Fyear[[simperiod]]@fleetsegments[[i]]@MAXDAYS.average.annual))) 
# -----------------------------------------------------------------------------
Fyear[[year_to_be_updated]]@fleetsegments[[i]]@technology <- convert_numeric0_to_na(as.numeric(as.character(Fyear[[simperiod]]@fleetsegments[[i]]@technology))) 
...
# -----------------------------------------------------------------------------
Fyear[[year_to_be_updated]]@fleetsegments[[i]]@employment.CI.perc[1, PERC] <- convert_numeric0_to_na(as.numeric(as.character(eimat[7,i]))) * as.numeric(as.character(Fyear[[year_to_be_updated]]@fleetsegments[[i]]@VESSELS.annual))
		}
...
if ( as.character(Fyear[[year_to_be_updated]]@fleetsegments[[i]]@landing_obligation[j]) == "Y") { 
      Fyear[[year_to_be_updated]]@fleetsegments[[i]]@revenues.landed_discard[j] <- convert_numeric0_to_na(as.numeric(as.character(Fyear[[year_to_be_updated]]@fleetsegments[[i]]@discard.weight[j]))) * as.numeric(as.character(Fyear[[year_to_be_updated]]@fleetsegments[[i]]@price.landed_discard[j]))      
      } else {
...
Fyear[[year_to_be_updated]]@fleetsegments[[i]]@revenues.landing.CI.perc[PERC,j] <- convert_numeric0_to_na(as.numeric(as.character(Fyear[[year_to_be_updated]]@fleetsegments[[i]]@landing.weight.CI.perc[PERC,j]))) * as.numeric(as.character(Fyear[[year_to_be_updated]]@fleetsegments[[i]]@price.CI.perc[PERC, j]))		 
		 
if ( as.character(Fyear[[year_to_be_updated]]@fleetsegments[[i]]@landing_obligation[j]) == "Y") {
      Fyear[[year_to_be_updated]]@fleetsegments[[i]]@revenues.landed_discard.CI.perc[PERC,j] <- convert_numeric0_to_na(as.numeric(as.character(Fyear[[year_to_be_updated]]@fleetsegments[[i]]@discard.weight.CI.perc[PERC,j]))) * as.numeric(as.character(Fyear[[year_to_be_updated]]@fleetsegments[[i]]@price.landed_discard.CI.perc[PERC, j]))		  
} else {
...
equation_type_COL1 <- convert_numeric0_to_na(as.numeric(as.character(eimat[1,i])))

	 if (equation_type_COL1 == 1) {
	 # read col 2 and 3
	 correction_factor_landing_COL2 <- convert_numeric0_to_na(as.numeric(as.character(eimat[2,i])))
	  correction_factor_revenues_COL3 <- convert_numeric0_to_na(as.numeric(as.character(eimat[3,i])))
	...
	Fyear[[year_to_be_updated]]@fleetsegments[[i]]@total.revenues.landings  <- correction_factor_revenues_COL3 *  sum(as.numeric(as.character( Fyear[[year_to_be_updated]]@fleetsegments[[i]]@revenues.landing)) , na.rm=T) 
	 ...
	 coeff_factor_landing_v_COL4 <- convert_numeric0_to_na(as.numeric(as.character(eimat[4,i])))
	  coeff_factor_landing_u_COL5 <- convert_numeric0_to_na(as.numeric(as.character(eimat[5,i])))
