# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/bmtgui/diagnosis/diagnosis_tab_impactpressure.r'), '\n')
...
img_yield_pb <- try(gdkPixbufNewFromFileTrim(img_yield_path), silent = TRUE)


yield_sw <<- try(gtkScrolledWindowNew(NULL, NULL),silent = T)
yield_sw$setShadowType("etched-in")
yield_sw$setPolicy("automatic", "automatic")
yield_sw$SetUsize(350, 350)    

try(
if (!inherits(img_yield_pb,'try-error')) {
  
img_yield_resized <- gdkPixbufScaleSimple(img_yield_pb, 300, 300, "bilinear")
img_yield <- gtkImageNewFromPixbuf(img_yield_resized)
...
Fmort_sw <<- try(gtkScrolledWindowNew(NULL, NULL),silent = T)
