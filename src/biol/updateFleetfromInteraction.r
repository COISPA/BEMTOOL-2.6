# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/updateFleetfromInteraction.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: updateFleetfromInteractionFyear, year_to_be_updated) {')
...
# if (phase != "SIMULATION") {         
# Fyear[[year_to_be_updated]]@fleetsegments[[i]]@total.landings.CI.perc[1, PERC] <- convert_numeric0_to_na(as.numeric(as.character(eimat[1,i]))) * as.numeric(as.character(Landingsfleet_perc[PERC,i]))   # total landings
# } else {
#  Fyear[[year_to_be_updated]]@fleetsegments[[i]]@total.landings.CI.perc[1, PERC] <- convert_numeric0_to_na(as.numeric(as.character(Fyear[[year_to_be_updated]]@fleetsegments[[i]]@total.landings)))   # total landings
# }
# 
# 
...
# _______________________________________________________________________________________________________    
 ...
 Fyear[[year_to_be_updated]]@fleetsegments[[i]]@total.landings <- convert_numeric0_to_na(as.numeric(as.character(eimat[1,i]))) * as.numeric(as.character(Landingsfleet[i]))   # total landings
             fleet_interaction_ord <- fleet_interaction_ord + 1      
            } else {
      Fyear[[year_to_be_updated]]@fleetsegments[[i]]@landing.weight[j] <- 0
      }
...
#Fleetyear[[1]]@fleetsegments[[flottaN_caso]]@landing.number
 ...
 Fyear[[year_to_be_updated]]@fleetsegments[[i]]@total.landings <- correction_factor_landing_COL2 * as.numeric(as.character(Landingsfleet[i]))   # total landings
	 ...
	 coeff_factor_landing_u_COL4 <- convert_numeric0_to_na(as.numeric(as.character(eimat[4,i])))
	  coeff_factor_landing_v_COL5 <- convert_numeric0_to_na(as.numeric(as.character(eimat[5,i])))
