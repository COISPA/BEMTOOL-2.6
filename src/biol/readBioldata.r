# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/readBioldata.r'), '\n')
...
Populations[[m_int]]@maturity.vect[1,]  <- convert_numeric0_to_na(as.numeric(as.character(Populations[[m_int]]@maturity.vect[1,] ))) / length(which(missing_SA[m_int,] == "Y"))
Populations[[m_int]]@maturity.vect[2,]  <- convert_numeric0_to_na(as.numeric(as.character(Populations[[m_int]]@maturity.vect[2,] ))) / length(which(missing_SA[m_int,] == "Y"))
...
#  print("***************************************************************************", quote=FALSE) 
  ...
  for (nr in c(1:4)) {
        ...
        for (year_rp in 1:length(years)) {
            rp_F.01[year_rp,] <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[year_rp]][[m_int_rp]]@referencePoints@F0.1[nr,])))
            rp_F.02[year_rp,] <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[year_rp]][[m_int_rp]]@referencePoints@F0.2[nr,])))
            rp_FMSY[year_rp,] <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[year_rp]][[m_int_rp]]@referencePoints@FMSY[nr,])))   
            rp_Fmax[year_rp,] <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[year_rp]][[m_int_rp]]@referencePoints@Fmax[nr,]))) 
        } 
