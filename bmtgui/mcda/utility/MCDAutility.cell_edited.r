# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/bmtgui/mcda/utility/MCDAutility.cell_edited.r'), '\n')
...
MCDAutility.cell_edited <- function(cell, path.string, new.text, data) {
 ...
 MCDAutility.model <- data 
  ...
  #	print(pproductions[[i]])
  	MCDAutility_list[[i]][column+1] <<- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(new.text ))))         # [column+1]
  ...
  MCDAutility_table[r,] <<- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(MCDAutility_list[[r]]))))
  }
