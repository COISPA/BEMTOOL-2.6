# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/econ/core/BMTFixcost.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: BMTFixcostoption, Flyear, fcmat, currenttime, n_fleet, tsmat3) {')

  if (option[4] == 1) { # Birdmod
    for (i in 1:n_fleet) {
    
    bmtfixedcost_maintenance_COL1 <- convert_numeric0_to_na(as.numeric(as.character(fcmat[1,i])))
    bmtfixedcost_other_COL2 <- convert_numeric0_to_na(as.numeric(as.character(fcmat[2,i])))
    bmtfixedcost_curr_GT <- convert_numeric0_to_na(as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@GT.annual )))
    bmtfixedcost_curr_newequip_cost <- as.numeric(as.character(tsmat3[i, (currenttime-simperiod)] ))
    bmtfixedcost_curr_vessel <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[currenttime]]@fleetsegments[[i]]@VESSELS.annual )))
...
#      Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost$tot.fix.cost <- sum(as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost$other.fix.cost)), as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost$new.equipment.cost)), na.rm=TRUE)
 Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost$tot.fix.cost <- convert_numeric0_to_na(as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost$other.fix.cost)))
...
#      Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost.CI.perc$tot.fix.cost[PERC] <- sum(as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost.CI.perc$other.fix.cost[PERC])), as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost.CI.perc$new.equipment.cost[PERC])), na.rm=TRUE)
 Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost.CI.perc$tot.fix.cost[PERC] <- convert_numeric0_to_na(as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost.CI.perc$other.fix.cost[PERC])))
			}
  ...
  if (option[4] == 2) { # Mefisto
    ...
    bmtfixedcost_un_maintenance_COL1 <- convert_numeric0_to_na(as.numeric(as.character(fcmat[1,i])))
    bmtfixedcost_essential_COL2 <- convert_numeric0_to_na(as.numeric(as.character(fcmat[2,i])))
    bmtfixedcost_av_maintenance_COL3 <- convert_numeric0_to_na(as.numeric(as.character(fcmat[3,i])))
    bmtfixedcost_curr_vessel <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[currenttime]]@fleetsegments[[i]]@VESSELS.annual  )))
    bmtfixedcost_prev_profit <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[currenttime-1]]@fleetsegments[[i]]@profit )))
    bmtfixedcost_curr_newequip_cost <- as.numeric(as.character(tsmat3[i, (currenttime-simperiod)] )) 
...
Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost$tot.fix.cost <- convert_numeric0_to_na(as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost$ess.cost)))
Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost$maint.cost <- sum(as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost$avoid.main.cost)), as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost$unavoid.main.cost)), na.rm=TRUE)   
    ...
    bmtfixedcost_prev_profit <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[currenttime-1]]@fleetsegments[[i]]@profit.CI.perc[PERC] )))
...
Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost.CI.perc$tot.fix.cost[PERC] <- convert_numeric0_to_na(as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost.CI.perc$ess.cost[PERC])))
Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost.CI.perc$maint.cost[PERC] <- sum(as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost.CI.perc$avoid.main.cost[PERC])), as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@fixed.cost.CI.perc$unavoid.main.cost[PERC])), na.rm=TRUE)       
  ...
  if (option[4] == 3) { # Fishrent     
    for (i in 1:n_fleet) {
      ...
      bmtfixedcost_total_COL1 <- convert_numeric0_to_na(as.numeric(as.character(fcmat[1,i])))
      bmtfixedcost_curr_vessel <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[currenttime]]@fleetsegments[[i]]@VESSELS.annual  )))
      bmtfixedcost_curr_newequip_cost <- as.numeric(as.character(tsmat3[i, (currenttime-simperiod)] ))
