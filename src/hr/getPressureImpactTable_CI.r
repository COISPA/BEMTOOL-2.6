# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/hr/getPressureImpactTable_CI.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: getPressureImpactTable_CI) {')
...
production_table_percentiles <- suppressWarnings(try(read.table(paste(PRODUCTION_table_CI, " quantiles.csv", sep=""),header=TRUE,sep=";"),silent=T) )
...
# landings -----------------------------------------------------------------------------
landing_w_value <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[yea]][[ss]]@interactions[[ff_ord]]$landings@totalweight.CI.perc )))    # [1,PERC]
...
# discard -----------------------------------------------------------------------------
discard_w_value <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[yea]][[ss]]@interactions[[ff_ord]]$discard@totalweight.CI.perc))) 
...
# mean lengths
catch_meanlength_value <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[yea]][[ss]]@interactions[[ff_ord]]$catches@meanLength.CI.perc))) 
...
discard_meanlength_value <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[yea]][[ss]]@interactions[[ff_ord]]$discard@meanLength.CI.perc))) 
...
#F_annual_value <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[yea]][[ss]]@mortalities$F[ff])))
#} else {
#F_annual_value <- NA
#}
...
if (ff_ord==1) {
# fford <- 1
totalD <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[yea]][[ss]]@totaldiscard@totalweight.CI.perc )))     # ''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''''?''
totalY <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[yea]][[ss]]@totalcatch@totalweight.CI.perc )))
totalL <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[yea]][[ss]]@totallanding@totalweight.CI.perc )))
totalF <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[yea]][[ss]]@totalcatch@fishing_mortality.CI.perc )))
...
#harvestRatio_value <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[yea]][[ss]]@exploitedStock@harvestRate.CI.perc)))
