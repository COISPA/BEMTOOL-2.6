# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/econ/core/BMTVarcost.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: BMTVarcostoption, Flyear, vcmat, vcvec , currenttime, n_fleet) {')
  ...
  if (option[[2]] == 1) { # Birdmod    
    for (i in 1:n_fleet) {
       ...
       bmtvarcost_fuel_COL1 <- convert_numeric0_to_na(as.numeric(as.character(vcmat[1,i])))
       bmtvarcost_commercial_COL2 <- convert_numeric0_to_na(as.numeric(as.character(vcmat[2,i])))
       bmtvarcost_other_COL3 <-  as.numeric(as.character(vcmat[3,i]))
       bmtvarcost_curr_days <- convert_numeric0_to_na(as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@DAYS.annual )))
       bmtvarcost_curr_landings <- convert_numeric0_to_na(as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@total.landings )))
  ...
  Flyear[[currenttime]]@fleetsegments[[i]]@variable.cost$tot.var.cost <- sum(as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@variable.cost$fuel.cost)), as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@variable.cost$commercial.cost)), as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@variable.cost$other.var.cost)), na.rm=TRUE)
     ...
     print(paste("COMMERCIAL COSTS ---------- YEAR:", currenttime-simperiod, "FLEET:", i, " coeff ==== ", bmtlabour_sorting_coeff_COL6))   
		...
		for (PERC in c(1:5)) {
		bmtvarcost_curr_landings <- convert_numeric0_to_na(as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@total.landings.CI.perc[PERC] )))
  ...
  if (option[[2]] == 2) { # Mefisto
    for (i in 1:n_fleet) {
      ...
      bmtvarcost_ice_COL4 <-  as.numeric(as.character(vcmat[4,i]))
       bmtvarcost_curr_days <- convert_numeric0_to_na(as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@DAYS.annual )))
       bmtvarcost_curr_revenues <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[currenttime]]@fleetsegments[[i]]@total.revenues )))
      
      bmtvarcost_fuel_timeseries <- convert_numeric0_to_na(as.numeric(as.character(vcvec[currenttime-simperiod])))
      ...
      Flyear[[currenttime]]@fleetsegments[[i]]@variable.cost$tot.var.cost <- sum( as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@variable.cost$fuel.cost)), as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@variable.cost$commercial.cost)),  as.numeric(as.character(Flyear[[currenttime]]@fleetsegments[[i]]@variable.cost$other.var.cost)), na.rm=TRUE)
				...
				for (PERC in c(1:5)) {
	  bmtvarcost_curr_revenues <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[currenttime]]@fleetsegments[[i]]@total.revenues.CI.perc[PERC] )))
