cat('Running script:', 'bmt_applyAutomaticReduction.r', '\n')
# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/bmtgui/scenarios/scenario_r4/bmt_applyAutomaticReduction.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: bmt_applyAutomaticReductionw) {')
...
#write.table(new_reduced_matrix, "C://test_automatic_reduction.csv", sep=";", row.names=F)                                                                                                    
  # B13-B13*(1-$B$29/100)
new_reduced_matrix[nrow(new_reduced_matrix), 2:13] <- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(new_reduced_matrix[1, 2:13])))) - as.numeric(as.character(new_reduced_matrix[1, 2:13])) * (1 - percentage_reduction_r4 / 100)
...
#       print(BMT_YEARS_FORECAST[yer]-BMT_YEARS_FORECAST[1])
    ...
    new_reduced_matrix[yer,2:13]  <- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(new_reduced_matrix[yer_prev, 2:13])))) - as.numeric(as.character(new_reduced_matrix[nrow(new_reduced_matrix),2:13])) / timespan_reduction_r4 
...
# if (printOK)   print('Calling function: bmt_resetAutomaticReductionw) {')
