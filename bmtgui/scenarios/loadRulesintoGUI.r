# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/bmtgui/scenarios/loadRulesintoGUI.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: loadRulesintoGUI) {')
...
#gtkComboBoxSetActive(bmt_combo_fleetsegments_effort_r4, 0)   
      ...
      gtkEntrySetText(bmt_entry_timespan_r5 ,  mat_cfg_scenario_settings_fore_options$HR3.timespan) 
             ...
             input_table_r5[,2] <<- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na( input_table_r5[,2]))))
             input_table_r5[,3] <<- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na( input_table_r5[,3]))))
             input_table_r5[,4] <<- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na( input_table_r5[,4]))))
           ...
           gtkToggleButtonSetActive(bmt_chk_hr_change_total_f, T)
            ...
            gtkEntrySetText(bmt_entry_timespan_r6 ,  mat_cfg_scenario_settings_fore_options$HR4.timespan) 
             ...
             input_table_r6[,3] <<- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na( input_table_r6[,3]))))
             input_table_r6[,4] <<- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na( input_table_r6[,4]))))
             ...
             input_table_r6_species_settings[,2] <<- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na( input_table_r6_species_settings[,2]))))
             input_table_r6_species_settings[,3] <<- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na( input_table_r6_species_settings[,3]))))
             input_table_r6_species_settings[,4] <<- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na( input_table_r6_species_settings[,4]))))
