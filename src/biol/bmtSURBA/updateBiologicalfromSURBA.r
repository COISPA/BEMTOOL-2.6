# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtSURBA/updateBiologicalfromSURBA.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: updateBiologicalfromSURBApopuls, IsY, source_obj, yy, mm, mort_const, year_interact) {')
if (FALSE) {
populs <- Populations
source_obj <- SURBARes
...
IsY[[year_interact]][[mm]]@mortalities[(length(BMT_FLEETSEGMENTS)+1),1] <- convert_numeric0_to_na(as.numeric(as.character(source_obj$total_mortality[yy,5]))) # Z
