# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/econ/core/BMTLandsplit.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: BMTLandsplitFlyear, Inters) {')
...
catch_table <- read.csv(catch_table_path,sep=";") 
    ...
    CPUE[i] <- convert_numeric0_to_na(as.numeric(as.character(Flyear[[simperiod]]@fleetsegments[[i]]@landing.weight[m_spe]))) / as.numeric(as.character(Flyear[[simperiod]]@fleetsegments[[i]]@GT.DAYS.annual))
    Eff_vect[i] <- Flyear[[yy]]@fleetsegments[[i]]@GT.DAYS.annual
    }
    for (i in 1:length(BMT_FLEETSEGMENTS)) { 
     # CPUE[i] <- convert_numeric0_to_na(as.numeric(as.character(Flyear[[1]]@fleetsegments[[i]]@landing.weight[m_spe]))) / as.numeric(as.character(Flyear[[1]]@fleetsegments[[i]]@GT.DAYS.annual))
