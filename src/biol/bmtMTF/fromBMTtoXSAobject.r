# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtMTF/fromBMTtoXSAobject.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: fromBMTtoXSAobjectXSAi) {')
    ...
    for (y_int in 1:simperiod) {
      ...
      if (!VIT.sex) {
              ...
              #  num_classes <- max(ages_F, ages_M) 
               ...
               # m 
               fV_Z <- convert_numeric0_to_na(as.numeric(as.character((resultsVit$VPA_results_mortalities   [1:num_classes, 2])))) 
               fV_F <- convert_numeric0_to_na(as.numeric(as.character((resultsVit$VPA_results_mortalities   [1:num_classes, 3])))) 
               diff_Z_F <-  fV_Z - fV_F
    ...
    # VIT by sex  
      ...
      # results of females
             ...
             mat_slot_M[length(mat_slot_M)] <- mean(as.numeric(as.character(resultsVit_M$ age_stocks  [min_num_classes:num_classes, 8] )) )    
               ...
               fV_Z_M <- convert_numeric0_to_na(as.numeric(as.character((resultsVit_M$VPA_results_mortalities   [1:num_classes, 2])))) 
               fV_F_M <- convert_numeric0_to_na(as.numeric(as.character((resultsVit_M$VPA_results_mortalities   [1:num_classes, 3])))) 
     ...
     # print(paste("(stock assessment tool = ",SAtool, ")", sep=""), quote=FALSE)
        ...
        bmtXSAstock@name  <- paste("Index File", BMT_SPECIES[m_int], "in GSA", BMT_GSA)
        bmtXSAstock@desc <- paste("Generated by BEMTOOL software",Sys.Date() )
        minAge <- convert_numeric0_to_na(as.numeric(as.character(resultsReport$age_rangeF$min))) 
        maxAge <- convert_numeric0_to_na(as.numeric(as.character(resultsReport$age_rangeF$max))) 
       ...
       num_classes <- max(as.numeric(as.character(Populations[[m_int]]@lifespan$lifespan))) 
            ...
            for (yea in 1:simperiod) {                              
               # mean weight for catch                                                                             rep(Interactionsyear[[yea]][[m_int]]@totalcatch@meanWeight , num_classes)
                if (!exists("catch_mean_weight_slot")) {  catch_mean_weight_slot <- convert_numeric0_to_na(as.numeric(as.character(cw_sw  )))
...
#        }
        ...
        bmtXSAstock@name  <- paste("Index File", BMT_SPECIES[m_int], "in GSA", BMT_GSA)
        bmtXSAstock@desc <- paste("Generated by BEMTOOL software",Sys.Date() )
        minAge <- convert_numeric0_to_na(as.numeric(as.character(max(as.numeric(ALADYM_GUI_simulations[[m_int]]@fishingmortality$min))))) 
        maxAge <- convert_numeric0_to_na(as.numeric(as.character(min(as.numeric(ALADYM_GUI_simulations[[m_int]]@fishingmortality$max))))) 
        range_vector <- c(0,(num_classes-1),NA,years[1],years[simperiod],minAge,maxAge)
        names(range_vector) <- c("min", "max", "plusgroup", "minyear", "maxyear", "minfbar", "maxfbar")
        bmtXSAstock@range <-  range_vector
        save_path <- paste(casestudy_path, "/", harvest_rule_id,"/Biological Pressure Impact/MSTF - ", BMT_SPECIES[m_int],"/", casestudy_name, " - ", BMT_SPECIES[m_int], " XSA object FORE ", harvest_rule_id,".dat", sep="")   
