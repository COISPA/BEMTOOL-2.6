# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/bmtgui/scenarios/loadScenariocfg_bk.r'), '\n')
...
EFFORT_DAY_list_fore <<- list()
    ...
    # print(mat_cfg_MEY)
     ...
     BMT_SCENARIO <<- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(mat_cfg_scenario_settings_fore[2,2])))) 
...
for (flee in 1:length(BMT_FLEETSEGMENTS)) {
  print(flee)
       cost_crew_minwage_matrix[1, flee+1] <- convert_numeric0_to_na (as.numeric(as.character(matrix_temp[as.character(matrix_temp[,1]) == paste("casestudy.labCostFun.F", flee, sep="") ,2] )))
       
         cost_crew_minwage_matrix[2, flee+1] <- convert_numeric0_to_na (as.numeric(as.character(matrix_temp[as.character(matrix_temp[,1]) == paste("casestudy.labCostFun.F", flee, sep="") ,6] )))

       labour_sorting_matrix[1, flee+1] <- convert_numeric0_to_na (as.numeric(as.character(matrix_temp[as.character(matrix_temp[,1]) == paste("casestudy.labCostFun.F", flee, sep="") ,7] )))
...
for (flee in 1:length(BMT_FLEETSEGMENTS)) {
       dyn_matrix[1, flee+1] <- convert_numeric0_to_na (as.numeric(as.character(matrix_temp[as.character(matrix_temp[,1]) == paste("casestudy.fleDynFun.F", flee, sep="") ,2] )))
         dyn_matrix[2, flee+1] <-convert_numeric0_to_na ( as.numeric(as.character(matrix_temp[as.character(matrix_temp[,1]) == paste("casestudy.fleDynFun.F", flee, sep="") ,3] )))
         dyn_matrix[3, flee+1] <-convert_numeric0_to_na ( as.numeric(as.character(matrix_temp[as.character(matrix_temp[,1]) == paste("casestudy.fleDynFun.F", flee, sep="") ,4] )))
}
...
for (flee in 1:length(BMT_FLEETSEGMENTS)) {
       progr_matrix[1, flee+1] <-convert_numeric0_to_na ( as.numeric(as.character(matrix_temp[as.character(matrix_temp[,1]) == paste("casestudy.tecProgFun.F", flee, sep="") ,2] )))
         progr_matrix[2, flee+1] <- convert_numeric0_to_na (as.numeric(as.character(matrix_temp[as.character(matrix_temp[,1]) == paste("casestudy.tecProgFun.F", flee, sep="") ,3] )))
         progr_matrix[3, flee+1] <- convert_numeric0_to_na (as.numeric(as.character(matrix_temp[as.character(matrix_temp[,1]) == paste("casestudy.tecProgFun.F", flee, sep="") ,4] )))
}
...
for (flee in 1:length(BMT_FLEETSEGMENTS)) {
      for (indi in 1:length(ECOIND_head)) {
       ecoind_matrix[indi, flee+1] <- convert_numeric0_to_na (as.numeric(as.character(matrix_temp[as.character(matrix_temp[,1]) == paste("casestudy.econIndFun.F", flee, sep="") ,(indi+1)] )))
}
}
...
for (flee in 1:length(BMT_FLEETSEGMENTS)) {
      for (ye in 1:length(BMT_YEARS_FORECAST)) {
       indic_taxes_matrix[flee, ye+1] <- convert_numeric0_to_na (as.numeric(as.character(matrix_temp[which(as.character(matrix_temp[,1]) == paste("casestudy.addtaxes.F", flee, sep="") ),(ye+1)] )))
}
}
