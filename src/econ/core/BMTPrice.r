# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/econ/core/BMTPrice.r'), '\n')
...
# Flyear=Fyear
# currenttime=year_to_be_updated


BMTPrice<- function(option, Flyear, pmat1, pmat2, pmat3, currenttime, n_fleet, m_stock) {
# if (printOK) if (printOK)   print('Calling function: BMTPriceoption, Flyear, pmat1, pmat2, pmat3, currenttime, n_fleet, m_stock) {')
...
# --------------------------------------------------------------------------------------------------------
  ...
  if (option[[1]] == 1) {  # Birdmod 
    for (i in 1:n_fleet) {
      ...
      bmtprice_landing_prevyear <- convert_numeric0_to_na(as.numeric(as.character(Flyear[[currenttime - 1]]@fleetsegments[[i]]@price[j] )))
      bmtprice_elastity_coeff_COL1 <- convert_numeric0_to_na(as.numeric(as.character( pmat1[j,i] )))

bmtprice_landing_weight_curryear <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[currenttime]]@fleetsegments[[i]]@landing.weight[j] )))
bmtprice_landing_weight_prevyear <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[currenttime - 1]]@fleetsegments[[i]]@landing.weight[j] )))
...
Flyear[[currenttime]]@fleetsegments[[i]]@price[j] <- bmtprice_landing_prevyear * ( 1 + bmtprice_elastity_coeff_COL1 *( (bmtprice_landing_weight_curryear - bmtprice_landing_weight_prevyear) / bmtprice_landing_weight_prevyear) ) 
      ...
      for (PERC in c(1:5)) {	      
      bmtprice_landing_prevyear <- convert_numeric0_to_na(as.numeric(as.character(Flyear[[currenttime - 1]]@fleetsegments[[i]]@price.CI.perc[PERC, j] )))
      bmtprice_elastity_coeff_COL1 <- convert_numeric0_to_na(as.numeric(as.character( pmat1[j,i] )))
     
       bmtprice_landing_weight_curryear <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[currenttime]]@fleetsegments[[i]]@landing.weight.CI.perc[PERC,j] )))
       bmtprice_landing_weight_prevyear <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[currenttime - 1]]@fleetsegments[[i]]@landing.weight.CI.perc[PERC,j] )))
...
#bmtprice_landing_meanweight_curryear <- convert_numeric0_to_na(as.numeric(as.character( Interactionsyear[[currenttime]][[j]]@interactions[[int_ass]]$landings@meanWeight ))) / 1000
bmtprice_landing_meanweight_curryear <- convert_numeric0_to_na(as.numeric(as.character( Interactionsyear[[currenttime]][[j]]@interactions[[int_ass]]$landings@meanWeight ))) 
bmtprice_lastSim_year <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[simperiod]]@fleetsegments[[i]]@price[j] ))) 
#print(paste("Peso medio",bmtprice_landing_meanweight_curryear))
#bmtprice_landing_weight_curryear <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[currenttime]]@fleetsegments[[i]]@landing.weight[j] )))
bmtprice_landing_weight_curryear <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[currenttime]]@fleetsegments[[i]]@landing.weight[j] ))) /1000

      bmtprice_elastity_coeff_COL1 <- convert_numeric0_to_na(as.numeric(as.character( pmat1[j,i] )))
      bmtprice_import_weight_curryear <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[currenttime]]@import.weight[j] )))
    ...
    bmtprice_lastSim_year <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[simperiod]]@fleetsegments[[i]]@price.CI.perc[1,1] ))) 

# bmtprice_landing_meanweight_curryear <- convert_numeric0_to_na(as.numeric(as.character( Interactionsyear[[currenttime]][[j]]@interactions[[int_ass]]$landings@meanWeight.CI.perc[1,PERC] ))) 
...
#bmtprice_landing_meanweight_curryear <- convert_numeric0_to_na(as.numeric(as.character( Interactionsyear[[currenttime]][[j]]@interactions[[int_ass]]$landings@meanWeight.CI.perc[1,PERC] ))) / 1000   # modifica fatta il 29/12/2017
 
# bmtprice_landing_weight_curryear <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[currenttime]]@fleetsegments[[i]]@landing.weight.CI.perc[PERC, j] )))       # modifica fatta il 29/12/2017
# bmtprice_landing_weight_curryear <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[currenttime]]@fleetsegments[[i]]@landing.weight.CI.perc[PERC, j] ))) /1000
...
#      bmtprice_landing_number_curryear <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[currenttime]]@fleetsegments[[i]]@landing.number[j] )))
#      } else {
#            bmtprice_landing_number_curryear <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[currenttime]]@fleetsegments[[i]]@landing.number[j] )))  +  as.numeric(as.character( Flyear[[currenttime]]@fleetsegments[[i]]@discard.number[j] ))
#      }
      bmtprice_elastity_coeff_COL3 <-  as.numeric(as.character(pmat3[j,i]))
...
# --------------------------------------------------------------------------------------------------------
    ...
    for (i in 1:n_fleet) {
      ...
      bmtprice_prevyear <- convert_numeric0_to_na(as.numeric(as.character(Flyear[[currenttime - 1]]@fleetsegments[[i]]@price[j] )))
      bmtprice_elastity_coeff_COL1 <- convert_numeric0_to_na(as.numeric(as.character( pmat1[j,i] )))
...
Flyear[[currenttime]]@fleetsegments[[i]]@price[j] <- bmtprice_prevyear * (bmtprice_landing_weight_curryear / bmtprice_landing_weight_prevyear)  
      ...
      for (PERC in c(1:5)) {	      
      bmtprice_prevyear <- convert_numeric0_to_na(as.numeric(as.character(Flyear[[currenttime - 1]]@fleetsegments[[i]]@price.CI.perc[PERC, j] )))
      bmtprice_elastity_coeff_COL1 <- convert_numeric0_to_na(as.numeric(as.character( pmat1[j,i] )))
...
# --------------------------------------------------------------------------------------------------------    
    ...
    for (i in 1:n_fleet) {
      ...
      bmtprice_price_landing_prevyear <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[simperiod]]@fleetsegments[[i]]@price[j] )))
        Flyear[[currenttime]]@fleetsegments[[i]]@price[j] <- bmtprice_price_landing_prevyear
        
        				for (PERC in c(1:5)) {
        		      bmtprice_price_landing_prevyear <- convert_numeric0_to_na(as.numeric(as.character( Flyear[[simperiod]]@fleetsegments[[i]]@price.CI.perc[PERC,j] )))
...
# --------------------------------------------------------------------------------------------------------    
      ...
      for (j in 1:m_stock) {
          ...
          price_land_COL1 <- convert_numeric0_to_na(as.numeric(as.character( pmat1[j,i] )))
