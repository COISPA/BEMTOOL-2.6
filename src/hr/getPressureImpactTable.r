# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/hr/getPressureImpactTable.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: getPressureImpactTableall_the_years) {')
...
if (ff_ord_inner == 1) {
   vect_catches <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[yea]][[ss]]@interactions[[ff_ord_inner]]$catches@totalweight )))
   vect_landings <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[yea]][[ss]]@interactions[[ff_ord_inner]]$landings@totalweight )))
} else {
...
catch_w_value <-  NA_integer_
}

if(length(catch_w_value)==0){
  catch_w_value=NA_integer_
...
landing_w_value <- NA_integer_
}

if(length(landing_w_value)==0){
  landing_w_value=NA_integer_
...
discard_w_value <-  NA_integer_
}

if(length(discard_w_value)==0){
  discard_w_value=NA_integer_
...
catch_meanlength_value <- NA_integer_
}

if(length(catch_meanlength_value)==0){
  catch_meanlength_value=NA_integer_
...
landing_meanlength_value <-  NA_integer_
}

if(length(landing_meanlength_value)==0){
  landing_meanlength_value=NA_integer_
...
if (ff_ord==1) {
# fford <- 1
totalD <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[yea]][[ss]]@totaldiscard@totalweight )))
totalY <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[yea]][[ss]]@totalcatch@totalweight)))
totalL <- convert_numeric0_to_na(as.numeric(as.character(Interactionsyear[[yea]][[ss]]@totallanding@totalweight )))   
...
if(length(Interactionsyear[[yea]][[ss]]@exploitedStock@exploitationRate)==0){
  Interactionsyear[[yea]][[ss]]@exploitedStock@exploitationRate=NA_integer_
