# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/bmtgui/economic_params/price/price_correction_fact/price_correction_fact.cell_edited.r'), '\n')
...
bmt_price_correction_fact.cell_edited <- function(cell, path.string, new.text, data) {
 ...
 bmt_price_correction_fact.model <- data 
  ...
  bmt_fleet.price_correction_fact[r,] <<- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(bmt_price_correction_fact_list[[r]]))))
  }
