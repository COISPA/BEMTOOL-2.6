# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/hr/calculateFcombined.r'), '\n')
calculateFcombined <- function(all_the_years,  path_pressure, path_eco) {
# if (printOK) if (printOK)   print('Calling function: calculateFcombinedall_the_years,  path_pressure, path_eco) {')
...
#colnames(Fref_combSUM_rev_byYear) <- c("Stock", "Year", "RevSpecies")
#
merged$prodFcurr_X_Revenues <- convert_numeric0_to_na(as.numeric(as.character(merged$Fcurrent))) * as.numeric(as.character(merged$RevenuesSpecies))
...
SUM_prodFcurr_X_Revenues$Fcombined <- convert_numeric0_to_na(as.numeric(as.character(SUM_prodFcurr_X_Revenues$SUM_prod_byFleet))) / as.numeric(as.character(SUM_prodFcurr_X_Revenues$SUM_revenues_byFleet))     # errore
